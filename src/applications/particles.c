...
//! Generate a uniform grid of particles with random shift of each particle.
/*! Similar to starsh_particles_generate_quasiuniform1(), but works only for
 * 1D, 2D and 3D. Number of locations `count/2` must be square of integer if
 * ndim`=2 and cube if integer if `ndim`=3.
 *
 * @param[out] data: Address of pointer to @ref STARSH_particles object.
 * @param[in] count: Amount of particles to generate.
 * @param[in] ndim: Dimensionality of space.
...
{      
	...
	{
		for(i = 0; i < count; i+=2)
			point[i] =  point[i+1] =(i+0.1+0.8*rand()/(1.0+RAND_MAX))/count;
	}
	else if(ndim == 2)
	{
	...
	{
		STARSH_int cbrtn = floor(cbrt(count/3)+0.1);
		if(cbrtn*cbrtn*cbrtn != count/3)
		{
			STARSH_ERROR("number of particles must be cube of some integer");
			return STARSH_WRONG_PARAMETER;
		}
		double *x = point, *y = x+count, *z = y+count;
		for(i = 0; i < 2*cbrtn; i+=2)
			for(j = 0; j < 2*cbrtn; j+=2)
				for(k = 0; k < 2*cbrtn; k+=2)
				{
					STARSH_int ind = (i*cbrtn + j)*cbrtn + k;
					x[ind] = x[ind+1]= (i+0.1+0.8*rand()/(1.0+RAND_MAX))/cbrtn;
					y[ind] = y[ind+1]= (j+0.1+0.8*rand()/(1.0+RAND_MAX))/cbrtn;
					z[ind] = z[ind+1]= (k+0.1+0.8*rand()/(1.0+RAND_MAX))/cbrtn;
				}
		zsort3(count, point);
	}
	STARSH_MALLOC(*data, 1);
...
//! Generate a uniform grid of particles with random shift of each particle.
/*! Similar to starsh_particles_generate_quasiuniform1(), but works only for
 * 1D, 2D and 3D. Number of locations `count/2` must be square of integer if `ndim`=2 and
 * cube if integer if `ndim`=3.
 *
 * @param[out] data: Address of pointer to @ref STARSH_particles object.
 * @param[in] count: Amount of particles to generate.
 ...
 * */
{
        ...
        {
                for(i = 0; i < count; i++)
                        point[i] = point[(count/2)+i] = (i+0.1+0.8*rand()/(1.0+RAND_MAX))/count;
        }
        else if(ndim == 2)
        {
        ...
        else
        {
                ...
                for(i = 0; i < cbrtn; i++)
                        for(j = 0; j < cbrtn; j++)
                                ...
                                {
                                        STARSH_int ind = (i*cbrtn + j)*cbrtn + k;
                                        x[ind] = x[(count/2)+ind] = (i+0.1+0.8*rand()/(1.0+RAND_MAX))/cbrtn;
                                        y[ind] = y[(count/2)+ind] = (j+0.1+0.8*rand()/(1.0+RAND_MAX))/cbrtn;
                                        z[ind] = z[(count/2)+ind] = (k+0.1+0.8*rand()/(1.0+RAND_MAX))/cbrtn;
                                }
                zsort3(count, point);
                zsort3(count/2, &point[count/2]);
        }
        STARSH_MALLOC(*data, 1);
        (*data)->point = point;
        (*data)->count = count;
...
static double uniform_distribution_fun(double rangeLow, double rangeHigh)
 ...
 * */
{
    ...
    {
        STARSH_int sqrtn = ceil(sqrt(count));
        /*if(sqrtn*sqrtn != count)
        {
            STARSH_ERROR("parameter `count` must be square of some integer");
            return STARSH_WRONG_PARAMETER;
